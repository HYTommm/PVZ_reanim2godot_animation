...
Result StartParamInit(R2GAStartParam* param)
{
	...
	strncpy(param->rootnodeType, "Node2D", NAME_LENTH - 1);
	param->rootnodeType[NAME_LENTH - 1] = '\0';
...
Result StartParamSetFromArgs(R2GAStartParam* param, int argc, char** argv)
{
	...
	strncpy(param->inputFileWholePath, inputFileWholePath, PATH_LENTH - 1);
		param->inputFileWholePath[PATH_LENTH - 1] = '\0';
	strncpy(param->animOutputGodotPath, animOutputGodotPath, PATH_LENTH - 1);
		param->animOutputGodotPath[PATH_LENTH - 1] = '\0';
	...
	strncpy(param->animOutputGodotPath, animOutputGodotPath, PATH_LENTH - 1); 
		param->animOutputGodotPath[PATH_LENTH - 1] = '\0';
	strncpy(param->resourceGodotPath, resourceGodotPath, PATH_LENTH - 1); 
		param->resourceGodotPath[PATH_LENTH - 1] = '\0';
	...
	for (int i = 4; i < argc; i++)
	{
		...
		if (strcmp(arg, "-of") == 0 || strcmp(arg, "--output-file") == 0)
		{
			...
			strncpy(param->outputFileWholePath, argv[i + 1], PATH_LENTH - 1);
			param->outputFileWholePath[PATH_LENTH - 1] = '\0';
		...
		if (strcmp(arg, "-cf") == 0 || strcmp(arg, "--config-file") == 0)
		{
			...
			strncpy(param->configFileWholePath, argv[i + 1], PATH_LENTH - 1);
			param->configFileWholePath[PATH_LENTH - 1] = '\0';
		...
		if (strcmp(arg, "-rnt") == 0 || strcmp(arg, "--root-node-type") == 0)
		{
			...
			strncpy(param->rootnodeType, argv[i + 1], NAME_LENTH - 1);
			param->rootnodeType[NAME_LENTH - 1] = '\0';
		...
		if (strcmp(arg, "-an") == 0 || strcmp(arg, "--anim-name") == 0)
		{
			...
			strncpy(param->animName, argv[i + 1], NAME_LENTH - 1);
			param->animName[NAME_LENTH - 1] = '\0';
...
Result ReadConfigFile(FILE* file, R2GAConfigParam configParams[], int maxConfigParams)
{
	...
	// 临时存储当前命令的token
	char tokens[MAX_TOKENS][NAME_LENTH];
	int token_count = 0;
		...
		else if (ch == ';')
		{
			...
			for (int i = 0; i < token_count - 1; i++) {
				if (param_count < maxConfigParams) {
					strncpy(configParams[param_count].key, tokens[i], NAME_LENTH - 1);
					configParams[param_count].key[NAME_LENTH - 1] = '\0';
					strncpy(configParams[param_count].value, value, NAME_LENTH - 1);
					configParams[param_count].value[NAME_LENTH - 1] = '\0';
					param_count++;
				}
			}
	...
	if (param_count <= maxConfigParams) {
		strncpy(configParams[param_count].key, "EndOfFile", NAME_LENTH - 1);
		configParams[param_count].key[NAME_LENTH - 1] = '\0';
		strncpy(configParams[param_count].value, "0", NAME_LENTH - 1);
		configParams[param_count].value[NAME_LENTH - 1] = '\0';
		param_count++;
	}
